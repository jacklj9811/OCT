...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pic_sum = {};
pic_one = [];	t = Tiff(filenames(1).name,'r');	imageData = read(t);	Y = imageData(:,:,1);	Y = Y(1:466,510:880); pic_one = double(Y);
pic_two = [];	t = Tiff(filenames(2).name,'r');	imageData = read(t);	Y = imageData(:,:,1);	Y = Y(1:466,510:880); pic_two = double(Y);
pic_three = [];	t = Tiff(filenames(3).name,'r');	imageData = read(t);	Y = imageData(:,:,1);	Y = Y(1:466,510:880); pic_three = double(Y);
mask = pic_one + pic_two + pic_three;	mask = mask./ max(max(mask));
mask(mask < prctile(mask, 20)) = -1;	mask(mask>0) = 0;	mask(mask<0) = 1;
pic_sum{1} = mask;
for ns_idx = 4:97
	pic_one = pic_two; pic_two = pic_three;
	t = Tiff(filenames(ns_idx).name,'r');
	imageData = read(t);
	Y = imageData(:,:,1);
	Y = Y(1:466,510:880);
...
for ns_idx = 1:97
	...
	if ns_idx <= 2
		...
		mask = pic_sum{ns_idx - 2};
	end
	tmpt(:,:,2) = mask;
	tmpt(:,:,3) = zeros(size(mask));
	% imshow(tmpt); title(['masked',filenames(ns_idx).name]); 
	% Y(mask>0) = 0;
	% Y = movmean(Y,6,2);
	edge_tmp = edge(Y,'canny',[],3);
	img = imageData(1:466,510:880,:);
	img(:,:,1) = img(:,:,1).*2;
	img(:,:,2) = edge_tmp.*255;
	imshow(img);title(['edge ',filenames(ns_idx).name]);
